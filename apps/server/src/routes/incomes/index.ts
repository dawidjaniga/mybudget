...
router.get(
  '/',
  errorBoundary(async function getIncomes (req, res) {
...
router.post(
  '/',
  errorBoundary(async function postIncomes (req, res) {
    const validPayload = await createIncomeSchema.validate(req.body)
    const category = await req.repository.getIncomeCategoryById(
